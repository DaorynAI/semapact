...
adds a small SemaPact-specific compatibility layer:
...
- SemaPact only adds a quality-derived `ALTER COLUMN ... SET NOT NULL` when
...
class SparkSqlContractExporter:
    ...
    def export_contract(
        ...
        *,
        ...
        unity_server_name: str = "semapact_target",
        ...
        SemaPact appends quality-derived constraint SQL only when the target
                ...
                if stmt:
                    ...
                    fmt = next(
                        (
                            ...
                            if p.property in ("semapact.table.format", "semapact.table.format")
                        ),
                        "delta",
                    )
                    stmt = f"{stmt}\nUSING {fmt}"
                    ...
                    loc = next(
                        (
                            p.value
                            for p in (schema_obj.customProperties or [])
                            if p.property in ("semapact.table.location", "semapact.table.location")
                        ),
                        None,
                    )
                    if loc:
